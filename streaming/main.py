...
BQ_DATASET = "dev_postgraduate_d1"
BQ_TABLE = "table1"
...
def _handle_duplication(db_ref):
    ...
    db_ref.update({"duplication_attempts": dups})
...
def _insert_into_bigquery(bucket_name, file_name):
    ...
    errors = BQ.insert_rows_json(
        table, json_rows=[row], row_ids=[file_name], retry=retry.Retry(deadline=30)
    )
    if errors != []:
...
def _handle_success(db_ref):
    ...
    doc = {u"success": True, u"when": _now()}
    db_ref.set(doc)
...
def _handle_error(db_ref):
    ...
    doc = {u"success": False, u"error_message": message, u"when": _now()}
    db_ref.set(doc)
